...
import { LayoutDashboard, UserRound, Type, FolderKanban, Layers3, BriefcaseBusiness, Save, RotateCcw, Download, Upload, ExternalLink, MonitorSmartphone, Undo2, CheckCircle2, Menu, X, LogOut, DatabaseZap } from 'lucide-react';
import { defaultContent, type SiteContent } from '@/lib/content';
import { loadContent, saveContent, resetContent, parseContent, readLegacyContent, clearLegacyContent } from '@/lib/content-store';
...
function Editor({ email }: { email: string }) {
  ...
  useEffect(() => {
    loadContent()
      .then(content => { setSaved(content); setDraft(content); setReady(true); setLegacy(readLegacyContent()); })
  ...
  const save = async () => {
    setStatus('saving');
    setSaveError('');
    try { const content = await saveContent(draft); setSaved(content); setDraft(content); setStatus('saved'); frame.current?.contentWindow?.location.reload(); setTimeout(() => setStatus('idle'), 2200); }
    catch (error) { setStatus('error'); setSaveError(error instanceof Error ? error.message : 'Erro ao salvar.'); }
  };
  const restore = async () => {
    if (!confirm('Restaurar todo o conteúdo para o padrão? O conteúdo publicado no site será substituído.')) return;
    try { const content = await resetContent(); setSaved(content); setDraft(content); frame.current?.contentWindow?.location.reload(); }
  ...
  const exportJson = () => {
    ...
    const link = document.createElement('a'); link.href = url; link.download = `portfolio-conteudo-${new Date().toISOString().slice(0, 10)}.json`; link.click();
    ...
    <div className="adm-main">
      <header className="adm-topbar">
        ...
        <div className="adm-top-actions">
          <span className={`adm-state ${dirty ? 'dirty' : ''}`} role="status">{status === 'saving' ? 'Salvando…' : status === 'saved' ? <><CheckCircle2 size={14}/> Salvo</> : status === 'error' ? `Erro: ${saveError}` : dirty ? 'Alterações não salvas' : saved.updatedAt ? `Salvo ${new Date(saved.updatedAt).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}` : 'Conteúdo padrão'}</span>
          ...
          <a className="adm-btn ghost" href="/" target="_blank" rel="noopener"><ExternalLink size={15}/> Ver site</a>
          <button className="adm-btn primary" disabled={!dirty || status === 'saving'} onClick={save}><Save size={15}/> Salvar</button>
        </div>
      </header>
      <div className="adm-body">
        <main className="adm-content adm-scroll" key={section}>
          {legacy && <div className="adm-banner"><DatabaseZap size={18}/><p>Encontramos conteúdo salvo neste navegador pela versão de demonstração. Quer trazê-lo para o rascunho? Depois é só clicar em <b>Salvar</b> para publicar no banco.</p><button className="adm-btn small primary" onClick={importLegacy}>Importar</button><button className="adm-btn small ghost" onClick={dismissLegacy}>Descartar</button></div>}
        ...
        {preview && <aside className="adm-preview"><div className="adm-preview-bar"><span>Prévia · reflete o que está salvo</span></div><iframe ref={frame} src="/" title="Prévia do site"/></aside>}
      </div>
    </div>
